...
namespace rage
{
    ...
    {
    public:
        char m_presence_attribute_key[64];
        union {
            char m_presence_attribute_string_value[256];
            uint64_t m_presence_attribute_int_value;
        };
        uint32_t m_presence_attibute_type;
        char pad_0144[4];
    };
...
<<<<<<< HEAD
        ...
        Submenu::Submenu("Network")
    {
        ...
        //auto adminDetection       = std::make_shared<Category>("Admin Detection");
...
>>>>>>> horsemenu-pr7
            ...
            if (g_PlayerDatabase->GetSelected() && show_player_editor)
            {
                ...
                if (ImGui::BeginChild("###selected_player", {500, static_cast<float>(*Pointers.ScreenResY - 388 - 38 * 4)}, false, ImGuiWindowFlags_NoBackground))
                {
                    ...
                    if (ImGui::InputScalar("RID", ImGuiDataType_S64, &current_player->rid, nullptr, nullptr, "%lld")
                    ...
                    {
                        ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "Warning: This player is a Rockstar Admin!");
                    }
...
>>>>>>> horsemenu-pr7
        ...
        //adminDetection->AddItem(adminGroup);
